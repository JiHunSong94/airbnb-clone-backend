...
class TestAmenity(APITestCase):
    ...
    def test_put_amenity(self):
        new_name = "New Test Amenity"
        new_desc = "New Test Des"
        ...
        response = self.client.put(
            "/api/v1/rooms/amenities/1", data={"description": new_desc}
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        ...
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["name"], new_name2)
        self.assertEqual(data["description"], new_desc2)
